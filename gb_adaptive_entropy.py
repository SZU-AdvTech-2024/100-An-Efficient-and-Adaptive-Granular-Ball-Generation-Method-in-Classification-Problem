...
def get_label_and_purity(gb):
    ...
    if len(len_label) == 1:
        ...
        # 计算数据分布熵
        label_counts = [sum(gb[:, 0] == l) for l in len_label]
        entropy = scipy.stats.entropy(label_counts)
    # print(label)
    # label, purity
    return label, purity, entropy
...
def plot_gb(granular_ball_list, plt_type=0):
    ...
    for granular_ball in granular_ball_list:
        label, p, entropy = get_label_and_purity(granular_ball)
...
def gb_plot(gb_dict, plt_type=0):
    ...
    for key in gb_dict.keys():
        ...
        label, p, entropy = get_label_and_purity(gb)
...
def splits(purity_init, gb_dict):
    ...
    while True:
        ...
        for key in gb_dict_temp.keys():
            ...
            gb = gb_single[key][0]
            p, purity, entropy = get_label_and_purity(gb)
            # print('p:', p)
...
def main():
    ...
    for d in range(len(keys)):
        ...
        for i in range(10):  # Repeat 10 times to get the average precision
            ...
            # center_init = train[:, 1:3].mean(axis=0)
            # print(center_init)

            distance_init = []
            for feature in train:
                # The distance from the initial center to the train
            ...
            label_cluster = k_means(X=train[:, 1:], n_clusters=splits_k, n_init=1, init=np.array(k_centers), random_state=5)[1]
            ...
            # plot_gb(ball_list)
            # plot_gb(ball_list, 1)
            count_num = evaluate_classification(ball_list, test)  # 测试精度
